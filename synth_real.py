...
# misaki (kokoro's EN/DE G2P) has no aarch64 wheels and we bypass KPipeline anyway —
# stub it so `import kokoro` (which pulls kokoro.pipeline) works on the GB10.
import types


class _StubModule(types.ModuleType):
    def __getattr__(self, name):  # serve any attribute (MToken, G2P, ...) as a dummy type
        if name.startswith("__"):  # let inspect/loguru see a normal module
            raise AttributeError(name)
        return type(name, (), {})


for _m in ("misaki", "misaki.en", "misaki.espeak"):
...
def notch_upsampler_tones(audio, sr=24000):
    """Remove the fine-tune-induced iSTFTNet upsampler whistles (2.4 kHz multiples).

    RUN2 forensics: fine-tuning drifts the decoder's checkerboard suppression,
    leaving steady tones at 2.4/4.8/7.2/9.6 kHz (up to +12 dB). Narrow notches
    remove them losslessly (verified: CER/DNSMOS unchanged, ridges <= -10 dB).
    """
...
def main():
    ...
    ap.add_argument("--texts", default=None,
                    help="file with one sentence per line (default: built-in TEST_SENTENCES)")
    ap.add_argument("--no-notch", action="store_true",
                    help="disable the upsampler-tone notch filters (production keeps them ON)")
    ...
    sentences = TEST_SENTENCES if not args.texts else \
        [l.strip() for l in open(args.texts, encoding="utf-8") if l.strip()]
    for i, text in enumerate(sentences, 1):
        ...
        if not args.no_notch:
            audio = notch_upsampler_tones(audio)
