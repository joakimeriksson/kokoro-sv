...
def main() -> None:
    ...
    ap.add_argument("--stride", type=int, default=1,
                    help="run N parallel workers: each handles lines where idx %% stride == offset")
    ...
    # parallel workers must not clobber each other's manifest; merge afterwards
    manifest = os.path.join(args.out, "speakers.csv" if args.stride == 1
                            else f"speakers_w{args.offset}.csv")
